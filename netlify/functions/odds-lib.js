...
async function fetchSport(key) {
  ...
  const url = 'https://site.web.api.espn.com/apis/v2/scoreboard/header?sport=' + encodeURIComponent(sport) + '&league=' + encodeURIComponent(league) + '&dates=' + dates;
  const data = await getJson(url);
  const pack = ((((data || {}).sports || [])[0] || {}).leagues || [])[0] || {};
  return (pack.events || []).map(ev => parseHeaderEvent(ev, key)).filter(Boolean);
}
