...
async function fetchSport(key) {
  ...
  if (!league) return [];
  const now = new Date();
  const tmr = new Date(now.getTime() + 2 * 86400000);
  const dates = ymd(now) + '-' + ymd(tmr);
  const base = 'https://site.web.api.espn.com/apis/v2/scoreboard/header?sport=' + encodeURIComponent(sport) + '&league=' + encodeURIComponent(league);
  let data = await getJson(base + '&dates=' + dates);
  let pack = ((((data || {}).sports || [])[0] || {}).leagues || [])[0] || {};
  let events = pack.events || [];
  if (!events.length) {
    data = await getJson(base);
    pack = ((((data || {}).sports || [])[0] || {}).leagues || [])[0] || {};
    events = pack.events || [];
  }
  return events.map(ev => parseHeaderEvent(ev, key)).filter(Boolean);
}
